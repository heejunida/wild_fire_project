...
FIRE_CSV = "fire_weather_merged_final.csv"
OUT_CSV = "fire_weather_merged_final_with_filled_start_fixed.csv"
OUT_XLSX = "fire_weather_merged_final_with_filled_start_fixed.xlsx"
API_SLEEP = 2.0
...
# (추가) 경고 방지용: date_start, hour_start 컬럼을 미리 문자열로 변환
if "date_start" in df.columns:
    df["date_start"] = df["date_start"].astype(str)
if "hour_start" in df.columns:
    df["hour_start"] = df["hour_start"].astype(str)

# 2. start 기후 데이터 비어있는 행만 찾아서 보완
for idx, row in df.iterrows():
    # 이미 값 있으면 skip
    if not (pd.isnull(row.get("T2M_start")) or row.get("T2M_start") == ""):
        continue

    lat = row["lat"]
    lng = row["lng"]
    yyyymmdd = f"{int(row['startyear']):04}{int(row['startmonth']):02}{int(row['startday']):02}"
    hour = hour_from_timestr(row["starttime"])

    print(f"[{idx+1}/{len(df)}] start값 보완: {yyyymmdd} {hour}시 ({lat},{lng})")

    weather = fetch_nasa_hourly_weather(lat, lng, yyyymmdd, hour)
    for k, v in weather.items():
        df.at[idx, f"{k}_start"] = v
    df.at[idx, "date_start"] = str(yyyymmdd)
    df.at[idx, "hour_start"] = str(hour)
    time.sleep(API_SLEEP)

# 3. 저장
df.to_csv(OUT_CSV, index=False, encoding="utf-8-sig")
print(f"\n최종 저장 완료: {OUT_CSV}")
df.to_excel(OUT_XLSX, index=False)  # encoding 옵션 필요 없음!
